...
public class Exercise23 extends JApplet {
	...
	class RectanglePanel extends JPanel {
		...
		public RectanglePanel() {
			addMouseListener(new MouseAdapter() {
				...
				public void mouseReleased(MouseEvent e) {
					if (!isInsideRectangle(e.getPoint())) { // Add a new circle
		...
		private boolean isInsideRectangle(Point p) {
		...
		protected void paintComponent(Graphics g) {
			...
			Graph<Rectangle> graph = new UnweightedGraph<Rectangle>(edges, rectangles);
			AbstractGraph<Rectangle>.Tree tree = graph.dfs(0); // a DFS tree
			boolean isAllCirclesConnected = rectangles.size() == tree
			...
			for (Rectangle rectangle : rectangles) {
				...
				if (isAllCirclesConnected) { // All circles are connected
					...
					g.fillRect(rectangle.x - size / 2, rectangle.y - size / 2, size, size);
				} else {
					g.drawRect(rectangle.x - size / 2, rectangle.y - size / 2, size, size);
				}
			}
		}
	}
	...
	private static class Rectangle {
		...
		public boolean contains(Point p) {
			if((p.x >= x - size / 2)&&(p.x <= x +  size / 2)&&(p.y >= y - size / 2)&&(p.y <= y + size / 2)) {
				return true;
			} else {
				return false;
			}
		...
		public boolean overlaps(Rectangle r) {
			if ((size >= Math.abs(this.x - r.x)) && (size >= Math.abs(this.y - r.y))) {
				return true;
			} else {
				return false;
			}
