...
public class Exercise01 {
    ...
    public static class RBTree<E extends Comparable<E>> extends BST<E> {
        ...
        public Tree24<E> getTree24() {
            Tree24<E> result = new Tree24();
            result.root = new Tree24.Tree24Node<E>();
            converNode((RBTreeNode<E>) root, result.root, result);
            return result;
        }

        private void converNode(RBTree<E>.RBTreeNode<E> rbTreeNode, Tree24.Tree24Node<E> tree24Node, Tree24<E> result) {
            if (rbTreeNode == null) {
                return;
            }
            tree24Node.elements = new ArrayList<>();
            tree24Node.child = new ArrayList<>();
            if (rbTreeNode.left != null) {
                if (((RBTreeNode<E>) (rbTreeNode.left)).isRed()) {
                    tree24Node.elements.add(rbTreeNode.left.element);
                    result.size++;
                    if(rbTreeNode.left.left != null) {
                        tree24Node.child.add(new Tree24.Tree24Node<E>());
                        converNode((RBTreeNode<E>) rbTreeNode.left.left, tree24Node.child.get(tree24Node.child.size() - 1), result);
                    }
                    if(rbTreeNode.left.right != null) {
                        tree24Node.child.add(new Tree24.Tree24Node<E>());
                        converNode((RBTreeNode<E>) rbTreeNode.left.right, tree24Node.child.get(tree24Node.child.size() - 1), result);
                    }                
                } else {
                    tree24Node.child.add(new Tree24.Tree24Node<E>());
                    converNode((RBTreeNode<E>) rbTreeNode.left, tree24Node.child.get(tree24Node.child.size() - 1), result);
                }
            }
            tree24Node.elements.add(rbTreeNode.element);
            result.size++;
            if (rbTreeNode.right != null) {
                if (((RBTreeNode<E>) (rbTreeNode.right)).isRed()) {
                    tree24Node.elements.add(rbTreeNode.right.element);
                    result.size++;
                    if (rbTreeNode.right.left != null) {
                        tree24Node.child.add(new Tree24.Tree24Node<E>());
                        converNode((RBTreeNode<E>) rbTreeNode.right.left, tree24Node.child.get(tree24Node.child.size() - 1), result);
                    }
                    if (rbTreeNode.right.right != null) {
                        tree24Node.child.add(new Tree24.Tree24Node<E>());
                        converNode((RBTreeNode<E>) rbTreeNode.right.right, tree24Node.child.get(tree24Node.child.size() - 1), result);
                    }
                } else {
                    tree24Node.child.add(new Tree24.Tree24Node<E>());
                    converNode((RBTreeNode<E>) rbTreeNode.right, tree24Node.child.get(tree24Node.child.size() - 1), result);
                }
            }
        }
         ...
         */
        protected class RBTreeNode<E extends Comparable<E>> extends BST.TreeNode<E> {
