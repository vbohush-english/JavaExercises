...
public class Exercise08 {
	  ...
	  public static void main(String[] args) {
		    ...
		    tree1.printTree();
		    List<Integer> hamiltonianCycle1 = graph1.getShortestHamiltonianCycle();
		    System.out.println(graph1.getPathWeight(hamiltonianCycle1) + "\t" + hamiltonianCycle1);
		    ...
		    tree2.printTree();
		    List<Integer> hamiltonianCycle2 = graph2.getShortestHamiltonianCycle();
		    System.out.println(graph2.getPathWeight(hamiltonianCycle2) + "\t" + hamiltonianCycle2);
	}
	...
	static class WeightedGraph<V> extends AbstractGraph<V> {
		...
		public List<Integer> getShortestHamiltonianCycle() {
			...
			for (int i = 0; i < vertices.size(); i++) {
				List<List<Integer>> allCycles = getAllCycles(i);
				for (int j = 0; j < allCycles.size(); j++) {
					if(vertices.size() == allCycles.get(j).size()) {
						if(result == null) {
							result = allCycles.get(j);
						} else {
							if(getPathWeight(allCycles.get(j)) < getPathWeight(result)) {
								result = allCycles.get(j);
							}
						}
					}
		...
		public double getPathWeight(List<Integer> path) {
			...
			for (int i = 0; i < newPath.size() - 1; i++) {
				int u = newPath.get(i);
				int v = newPath.get(i + 1);
				for (int j = 0; j < queues.size(); j++) {
					PriorityQueue<WeightedEdge> tmpQueue = queues.get(j);
					for (WeightedEdge weightedEdge : tmpQueue) {
						if((u == weightedEdge.u) && (v == weightedEdge.v)) {
		...
		public List<List<Integer>> getAllCycles(int u) {
			...
			getAllCycles(u, u, parent, searchOrder, isVisited, list);
			return list;
		}

		private void getAllCycles(int first, int v, int[] parent, List<Integer> searchOrder, boolean[] isVisited, List<List<Integer>> result) {
			searchOrder.add(v);
			isVisited[v] = true;
			...
			for (int i : neighbors.get(v)) {
				if (!isVisited[i]) {
					...
					getAllCycles(first, i, newParent, newSearchOrder, newIsVisited, result);
				} else if(first == i) {
