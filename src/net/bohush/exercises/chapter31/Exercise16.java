...
public class Exercise16 extends JApplet{
	...
	private AbstractGraph<String>.Tree path = null;
	...
	private void updateMST() {
		...
		path = graph.getMinimumSpanningTree();
	}
	...
	public static void main(String[] args) {
		JFrame jFrame = new JFrame("Exercise16");
		JApplet jApplet = new Exercise16();
		jFrame.add(jApplet);
		...
		protected void paintComponent(Graphics g) {
			...
			if(path != null) {
				...
				for (int i = 0; i < path.parent.length; i++) {
					if (path.parent[i] != -1) {
						int length = 20;
						int x0 = vertexes.get(i).x;
						int y0 = vertexes.get(i).y;
						int x1 = vertexes.get(path.parent[i]).x;
						int y1 = vertexes.get(path.parent[i]).y;
