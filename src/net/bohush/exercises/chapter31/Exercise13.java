...
public class Exercise13 extends JApplet {
	...
	public Exercise13() {
		...
		jPanel1.add(new JLabel("Ending City"));
		jPanel1.add(jTextField2);
		JButton jButton = new JButton("Display Shortest Path");
		jPanel1.add(jButton);
		add(jPanel1, BorderLayout.SOUTH);
		jButton.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				int city1 = getCityIndex(jTextField1.getText());
				int city2 = getCityIndex(jTextField2.getText());
				if(city1 == -1) {
					JOptionPane.showMessageDialog(null, "Wrong Starting City Name!", "Error", JOptionPane.ERROR_MESSAGE);
					return;
				}
				if(city2 == -1) {
					JOptionPane.showMessageDialog(null, "Wrong Ending City Name!", "Error", JOptionPane.ERROR_MESSAGE);
					return;
				}
				WeightedGraph<City>.ShortestPathTree shortestPathTree = graph.getShortestPath(city1);
				path = shortestPathTree.getPath(city2);
				graphView.repaint();
			}
		});
	}
	...
	class GraphView extends javax.swing.JPanel {
		...
		protected void paintComponent(java.awt.Graphics g) {
			...
			List<PriorityQueue<WeightedEdge>> weightededges =  graph.getWeightedEdges();
			ArrayList<WeightedEdge> all = new ArrayList<>();
			for (int i = 0; i < weightededges.size(); i++) {
				PriorityQueue<WeightedEdge> tmpQueue = weightededges.get(i);
			...
			for (int i = 0; i < graph.getSize(); i++) {
				...
				for (int v : neighbors) {
					...
					for (WeightedEdge weightedEdge : all) {
						if((weightedEdge.u == i) && (weightedEdge.v == v)) {
							g.drawString(weightedEdge.weight + "", (x1 + x2)/ 2, (y1 + y2) / 2);		
						}
					}					
				}
			}
			...
			if(path != null) {
				...
				for (int i = 0; i < path.size() - 1; i++) {
					int length = 20;
					int x0 = path.get(i).getX();
					int y0 = path.get(i).getY();
